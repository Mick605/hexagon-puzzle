...
class RoutingHandler {
    ...
    constructor() {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (path[0].tagName == 'A') {
                const url = new URL(path[0].href, this.currentUrl);
    ...
    activateOrDeactivateComp(comp, routeUrl) {
        ...
        if (match !== null) {
            ...
            if (comp.activate) setTimeout(() => comp.activate(match.endUrl, match.params), 0);
        } else {
            delete comp.dataset['routingMatch'];
            if (comp.deactivate) setTimeout(() => comp.deactivate(), 0);
        }
    }
